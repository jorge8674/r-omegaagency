...
const DEPT_BG: Record<string, string> = {
  marketing:  "bg-orange-500/5",  tech:       "bg-blue-500/5",
  operations: "bg-green-500/5",   finance:    "bg-emerald-500/5",
  community:  "bg-purple-500/5",  futures:    "bg-indigo-500/5",
  people:     "bg-pink-500/5",    security:   "bg-rose-500/5",
};
...
export function MarketplaceDirectorBar({ isAgentActive, onSolicitar }: Props) {
  ...
  return (
    <div className="flex items-start gap-3 overflow-x-auto pb-2 pt-2 scrollbar-thin">
      ...
      {directors.map((d) => {
        ...
        const bg = DEPT_BG[dept] ?? "bg-muted/5";
        const active = d.sub_agents.filter((a) => ["active","online","in_task"].includes(a.status)).length;
        const locked = !isAgentActive(d.code);

        return (
          <button
            key={d.id}
            onClick={() => locked && onSolicitar({
              code: d.code, name: d.name, department: d.department,
              role: "director", status: d.status, active: false,
            })}
            className={`relative overflow-visible shrink-0 flex flex-col items-center gap-1.5 rounded-xl border ${style.border} ${bg} shadow-sm ${style.glow} px-4 py-2.5 min-w-[100px] transition-all duration-150 cursor-pointer ${locked ? "opacity-60" : "hover:scale-[1.03]"}`}
          >
            {locked && <Lock className="absolute -top-2 -right-2 z-10 h-4 w-4 text-muted-foreground bg-background rounded-full p-0.5" />}
