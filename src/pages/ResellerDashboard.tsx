...
import {
  ...
  Ban, CheckCircle, AlertTriangle, UserCircle, Palette, Inbox,
} from "lucide-react";
import { AddClientModal } from "@/components/dashboard/AddClientModal";
import { LeadsTab } from "@/components/reseller/LeadsTab";
...
export default function ResellerDashboard() {
  ...
  const { data, isLoading } = useQuery<ResellerData>({
    ...
    <div className="space-y-6 relative pb-20">
      ...
      <div className="flex items-center gap-4">
        {isLoading ? (
          ...
          <img src={branding.logo_url} alt={reseller?.agency_name} className="h-10 max-w-[180px] object-contain" />
        ) : (
          <h1 className="font-display text-2xl font-bold tracking-tight">{reseller?.agency_name || "Agencia"}</h1>
        )}
        <Badge variant="outline" className="bg-primary/10 text-primary border-primary/30 text-xs">Panel de Agencia</Badge>
        <Button size="sm" variant="outline" onClick={() => navigate(`/reseller/branding?reseller_id=${resellerId}`)}>
      ...
      <div className={banner.blocking && !isLoading ? "opacity-30 pointer-events-none select-none" : ""}>
        <Tabs defaultValue="clients" className="w-full">
          ...
          <TabsContent value="clients" className="mt-6">
            <Card className="glass">
              <CardHeader className="flex flex-row items-center justify-between">
                ...
                <Button size="sm" className="gradient-primary text-primary-foreground" onClick={() => setAddClientOpen(true)}>
                    ...
                    <TableBody>
                      {clients.map((c) => (
                        <TableRow key={c.id}>
                          ...
                          <TableCell>
                            <Badge variant="outline" className="bg-secondary text-secondary-foreground">{c.plan || "Básico"}</Badge>
                          </TableCell>
                          <TableCell>
                            <Badge variant="outline" className={c.active ? "bg-success/20 text-success border-success/30" : "bg-muted text-muted-foreground"}>
                              {c.active ? "Activo" : "Inactivo"}
                            </Badge>
                          </TableCell>
                          <TableCell>
                            <Button size="sm" variant="ghost" className="text-primary" onClick={() => toast({ title: "Próximamente", description: `Dashboard de ${c.name} en desarrollo.` })}>
          ...
          <TabsContent value="team" className="mt-6">
            <Card className="glass">
              <CardHeader className="flex flex-row items-center justify-between">
                ...
                <Button size="sm" variant="outline" onClick={() => toast({ title: "Próximamente", description: "Función disponible próximamente." })}>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                    {agents.map((a) => (
                      <div key={a.id} className="flex items-center gap-3 rounded-lg border border-border bg-secondary/30 p-3">
                        <div className="flex h-9 w-9 items-center justify-center rounded-full bg-primary/10 text-primary font-bold text-sm">{a.name.charAt(0).toUpperCase()}</div>
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-medium truncate">{a.name}</p>
                          <p className="text-xs text-muted-foreground truncate">{a.email}</p>
                        </div>
                        <Badge variant="outline" className={a.status === "active" ? "bg-success/20 text-success border-success/30 text-xs" : "bg-muted text-muted-foreground text-xs"}>
          ...
          <TabsContent value="leads" className="mt-6">
            <LeadsTab resellerId={resellerId} />
          </TabsContent>
        </Tabs>
        ...
        <Card className="glass mt-6">
          ...
          <CardContent>
            {isLoading ? (
              <div className="space-y-3">{[...Array(4)].map((_, i) => <Skeleton key={i} className="h-6 w-48" />)}</div>
            ) : (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1">
                  <p className="text-xs text-muted-foreground">Status</p>
                  <Badge variant="outline" className={status === "active" ? "bg-success/20 text-success border-success/30" : status === "suspended" ? "bg-destructive/20 text-destructive border-destructive/30" : "bg-warning/20 text-warning border-warning/30"}>
                    {status === "active" ? "Activo" : status === "suspended" ? "Suspendido" : "Advertencia"}
                  </Badge>
                </div>
                <div className="space-y-1">
                  <p className="text-xs text-muted-foreground">Comisión Rate</p>
                  <p className="text-sm font-mono font-medium">{((reseller?.omega_commission_rate || 0.3) * 100).toFixed(0)}%</p>
                </div>
                <div className="space-y-1">
                  <p className="text-xs text-muted-foreground">Días en Mora</p>
                  <p className={`text-sm font-mono font-medium ${(reseller?.days_overdue || 0) > 0 ? "text-destructive" : ""}`}>{reseller?.days_overdue || 0}</p>
                </div>
                <div className="space-y-1">
                  <p className="text-xs text-muted-foreground">Próximo Pago</p>
                  <p className="text-sm font-mono font-medium">{reseller?.payment_due_date ? new Date(reseller.payment_due_date).toLocaleDateString("es-MX") : "—"}</p>
                </div>
              </div>
            )}
          </CardContent>
