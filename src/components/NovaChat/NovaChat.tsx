...
export function NovaChat({ onClose, onMinimize }: Props) {
  ...
  return (
    ...
    >
      ...
      {/* Tab Bar */}
      <div className="flex border-b border-border/20 px-3 pt-1.5">
        {(["chat", "context"] as Tab[]).map((t) => (
          <button
            key={t}
            onClick={() => setTab(t)}
            className={`flex items-center gap-1 px-3 py-1.5 text-[10px] font-semibold uppercase tracking-wider rounded-t-md transition-colors ${
              tab === t
                ? "text-yellow-400 border-b-2 border-yellow-400"
                : "text-muted-foreground hover:text-foreground"
            }`}
          >
            {t === "context" && <FileText className="h-3 w-3" />}
            {t === "chat" ? "Chat" : `Contexto${docs.length > 0 ? ` (${docs.length})` : ""}`}
          </button>
        ))}
      </div>
          ...
          <div className="flex-1 overflow-y-auto space-y-3 px-4 py-3 scrollbar-thin">
            {messages.length === 0 && (
              <div className="flex flex-col items-center justify-center h-full text-center text-muted-foreground">
                ...
                {docs.length > 0 && (
                  <p className="text-[9px] mt-2 text-yellow-400/60">📎 {docs.length} doc{docs.length > 1 ? "s" : ""} en contexto</p>
                )}
              </div>
            )}
            {messages.map((msg, i) => (
      ...
      {tab === "context" && (
        <div className="flex-1 overflow-hidden">
          <NovaChatContext docs={docs} onDocsChange={setDocs} />
        </div>
      )}
    </div>
  );
