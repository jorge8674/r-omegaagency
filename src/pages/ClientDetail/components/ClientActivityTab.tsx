...
import { Activity, LogIn, FileText, Bot, CreditCard } from "lucide-react";
import { formatDistanceToNow } from "date-fns";
import { es } from "date-fns/locale";
import type { ClientActivityItem } from "../hooks/useClientDetail";
...
export function ClientActivityTab({ activity, loading }: Props) {
  if (loading) {
  ...
  return (
    <div className="space-y-1">
      {safe.slice(0, 20).map((item) => {
        ...
        return (
          <div key={item.id}
            className="flex items-center gap-3 p-3 rounded-lg border border-border/20 bg-card/40 hover:border-yellow-500/20 transition-colors">
